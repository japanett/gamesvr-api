...
public class AdminGameService {
    ...
    @Transactional
    public GameEntity updateGamePlatforms (final String token, final Long gameId, final AddGamePlatformRequest request) {
        AdminEntity admin = tokenInterceptorService.translateAdminToken(token);
        final GameEntity game = this.adminGameRepository.findById(gameId)
                .orElseThrow(() -> new ResourceNotFoundException("AdminGameService, m=addGamePlatform, Game not found! id: " + gameId.toString()));
